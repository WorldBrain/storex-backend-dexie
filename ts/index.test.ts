...
describe('Dexie StorageBackend batch operations', () => {
    ...
    it('should support batches with updateObject operations', async () => {
        ...
        const { object: object1 } = await storageManager
            .collection('user')
            .createObject({ displayName: 'Jack' })
        const { object: object2 } = await storageManager
            .collection('user')
            .createObject({ displayName: 'Jane' })
        await storageManager.operation('executeBatch', [
            {
                operation: 'updateObjects',
                collection: 'user',
                where: { id: object1.id },
                updates: { displayName: 'Jack 2' },
            },
            {
                operation: 'updateObjects',
                collection: 'user',
                where: { id: object2.id },
                updates: { displayName: 'Jane 2' },
            },
        ])
        expect([
            await storageManager
                .collection('user')
                .findOneObject({ id: object1.id }),
            await storageManager
                .collection('user')
                .findOneObject({ id: object2.id }),
        ]).toEqual([
